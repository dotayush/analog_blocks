...
Constructed from an inductor and capacitor in series. An LC
...
An LC filter differs from an RC or RL filter in that it is a
second order filter. Each order adds -20 dB/decade to the
roll-off rate which gives us a steeper transition between
the pass band and stop band. Thus, a second order filter has a
...
\begin{enumerate}
  \item Due to \(V_{1}\) alone (\(V_{2} = 0V\)):
    ...
    \begin{align*}
      ...
      \boxed{H_1(s) = \frac{N_1(s)}{D_1(s)} = \frac{V_{out_1}}{V_{1}} = \frac{1}{s^2C_1L_1 + 1}}
    \end{align*}
  \item Due to \(V_{2}\) alone (\(V_{1} = 0V\)):
    ...
    \begin{align*}
      ...
      \boxed{H_2(s) = \frac{N_2(s)}{D_2(s)} = \frac{V_{out_2}}{V_{2}} = \frac{s^2C_1L_1}{s^2C_1L_1 + 1}}
    \end{align*}
\end{enumerate}
