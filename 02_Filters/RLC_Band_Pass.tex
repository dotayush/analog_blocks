\subsubsection{RLC Band Pass Filter}
...
\begin{enumerate}
  ...
  \item \textbf{Series Configuration:}
    ...
    Configuration, we get:
    \begin{align*}
      (V_{out} - V_{x})sC_1 + (V_{out} - V_{ee}){G_1} &= 0 \tag{1}
    \end{align*}
    Applying KCL at \(V_{x}\):
    \begin{align*}
      \frac{(V_{x} - V_{in})}{sL_1} + (V_{x} - V_{out})sC_1 &= 0 \tag{2}
    \end{align*}
    From equation (2):
    \begin{align*}
      ...
      V_{x} &= \frac{V_{in} + V_{out}s^2 L_1 C_1}{1 + s^2 L_1 C_1} \tag{3}
    \end{align*}
    \paragraph{Superposition:}
      \begin{enumerate}
        \item Due to \(V_{in}\) alone (\(V_{ee} = 0V\)):
