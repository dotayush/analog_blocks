...
Pass Filter can be broken down into two types: Low Pass and
High Pass.
\begin {itemize}
...
\end{itemize}
It is a single pole filter but can be used in multiple
stages to create higher order filters.
...
\begin{enumerate}
  \item Due to \(V_{1}\) alone (\(V_{2} = 0V\)):
    ...
    \begin{align*}
      ...
      \boxed{H_1(s) = \frac{N_1(s)}{D_1(s)} = \frac{V_{out_1}}{V_{1}} = \frac{G_1}{G_1 + sC_1}}
    \end{align*}
  \item Due to \(V_{2}\) alone (\(V_{1} = 0V\)):
    ...
    \begin{align*}
      ...
      \boxed{H_2(s) = \frac{N_2(s)}{D_2(s)} = \frac{V_{out_2}}{V_{2}} = \frac{sC_1}{G_1 + sC_1}}
    \end{align*}
\end{enumerate}
