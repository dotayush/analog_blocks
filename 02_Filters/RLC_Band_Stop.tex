\subsection{RLC Band Stop Filter}
...
\begin{enumerate}
  ...
  \item \textbf{Series Configuration:}
    ...
    Configuration, we get:
    \begin{align*}
      (V_{out} - V_{in})G_1 + \frac{(V_{out} - V_{x})}{sL_1} = 0 \tag{i}
    \end{align*}
    Applying KCL at \(V_{x}\):
    \begin{align*}
      (V_{x} - V_{ee})sC_1 + \frac{(V_{x} - V_{out})}{sL_1} = 0 \tag{ii}
    \end{align*}
    From equation (ii):
    \begin{align*}
      ...
      \boxed{V_{x} = \frac{V_{ee}s^2L_1C_1 + V_{out}}{s^2L_1C_1 + 1}} \tag{iii}
    \end{align*}
    \paragraph{Superposition:}
      \begin{enumerate}
        \item Due to \(V_{in}\) alone (\(V_{ee} = 0V\)):
