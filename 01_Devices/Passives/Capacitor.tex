...
\begin{itemize}
  \item \textbf{Capacitance:}
    ...
    \begin{align*}
      Q = C \cdot V \\
      \text{Integrating both sides with respect to time:} \\
  ...
  \item \textbf{Impedance:}
    ...
    \begin{align*}
      I &= C \cdot \frac{dV}{dt} \\
      I(s) &= C \cdot s (V(s) - V(0)) \hspace{1cm} [\text{Laplace Transform}] \\
      V(s) &= \frac{1}{sC} I(s) \hspace{1cm} [\text{assuming \(V(0) = 0\)}]
    \end{align*}
  ...
  \item \textbf{Circuit Analysis:}
    ...
    or DC) applied to it. Find the admittance before deciding weather to
