...
\paragraph{Overview:}
Constructed from two transistors and a resistor, a Widlar
Current Mirror is a variation of the basic current mirror
that allows for the generation of smaller output currents
than the reference current. You can adjust the output
current by changing the value of the resistor. The source
are sink configurations are complementary to each other.
Refer \ref{sec:theory_general_complementary} for information
on complementary circuits.
  ...
  \item \textbf{Large Signal Analysis}:\\
    ...
    \begin{align*}
      I_{M_0} &= k_{n_0}(V_{gs_0} - V_{th})^2 \cdot (1 + \lambda V_{ds_0}) \\
              ...
              &= k_{n_0}\left(\left(V_{cc} - \frac{(I_{M_0} + I_{G})}{G_2}\right) - V_{ee} - V_{th}\right)^2 \quad \text{[assuming } \lambda = 0 \text{]} \tag{iii} \\ \\
      I_{M_1} &= k_{n_1}(V_{gs_1} - V_{th})^2 \cdot (1 + \lambda V_{ds_1}) \\
              ...
              &= k_{n_1}\left(\left(V_{cc} - \frac{(I_{M_0} + I_{G})}{G_2}\right) - V_{ee} - V_{th}\right)^2 \quad \text{[assuming } \lambda = 0 \text{]} \tag{iv} \\
    \end{align*}
    It can be seen that,
    \begin{align*}
    ...
    Thus,
    \begin{align*}
      \boxed{\frac{I_{out}}{I_{in}} \approx \frac{I_{M_1}}{I_{M_0}} \approx \frac{k_{n_1}}{k_{n_0}} = \frac{\frac{\mu_n C_{ox} W_1}{L_1}}{\frac{\mu_n C_{ox} W_0}{L_0}} = \frac{\frac{W_1}{L_1}}{\frac{W_0}{L_0}} = \frac{W_1 L_0}{L_1 W_0}}
    \end{align*}
  \item \textbf{Small Signal Analysis}:\\
    Applying KCL at $V_{out}$:
    \begin{align*}
      V_{out}G_3 + V_{out}g_{o_1} + g_{m_1}V_{x} &= 0 \\
      V_{out}(G_3 + g_{o_1}) + g_{m_1}V_{x} &= 0 \tag{v}
    \end{align*}
    Applying KCL at $V_{x}$:
    \begin{align*}
      V_{x}G_2 + V_{x}g_{o_0} + g_{m_0}V_{x} + I_{G} &= 0 \\
      V_{x}(G_2 + g_{o_0} + g_{m_0}) &= 0 \quad \text{[assuming } I_G \approx 0 \text{ by MOS physics]} \tag{vi}
    \end{align*}
    Adding voltage source $V_s$ at $V_x$ that supplies
    a current $I_s$ into $V_{x}$, we get:
    \begin{align*}
      V_{x}(G_2 + g_{o_0} + g_{m_0}) - I_{s} &=0 \tag{vi'}
    \end{align*}
    Third equation arises from Voltage Source:
    \begin{align*}
      V_{x} = V_{s} \tag{vii}
    \end{align*}
    Solving,
    \begin{align*}
      V_{out} &= - \frac{g_{m_1} V_{x}}{G_3 + g_{o_1}} \\
              &= - \frac{g_{m_1} V_{s}}{G_3 + g_{o_1}} \quad \text{[from (vii)]} \\
      \boxed{H(s) = \frac{N(s)}{D(s)} = \frac{V_{out}}{V_{s}} = - \frac{g_{m_1}}{G_3 + g_{o_1}}}
    \end{align*}
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{Impedances}:
    ...
    \paragraph{At Output:} Applying a test current $I_{test}$
      at $V_{out}$, from equation (v):
      \begin{align*}
        V_{out}(G_3 + g_{o_1}) + g_{m_1}V_{x} &= I_{test} \\
        V_{out}(G_3 + g_{o_1}) + g_{m_1}(0) &= I_{test} \quad \text{[from (vii), } V_{x} = 0 \text{]} \\
...
\paragraph{Question:}
Design a Widlar Current Mirror (Sink) such that it
...
$L = 1 \mu m$ for all transistors. Find
the output impedance of the mirror and the maximum
load that can be connected at the output if $V_{L,
max} = 4V$.
\paragraph{Solution:}
