...
\paragraph{Overview:}
Constructed from transistors and resistors, a Wilson
Current Mirror is an improved version of the Widlar
Current Mirror that offers better output resistance
and accuracy in current mirroring.
...
\begin{enumerate}
  \item \textbf{Large Signal Analysis}:\\
    ...
    \begin{align*}
      ...
      V_{out} &= V_{cc} - \frac{I_{M_2}}{G_4} \tag{i}
    \end{align*}
    Applying KCL at $V_{y}$:
    \begin{align*}
      (V_{y} - V_{cc})G_3 + I_{M_0} &= 0 \\
      V_{y} &= V_{cc} - \frac{I_{M_0}}{G_3} \tag{ii}
    \end{align*}
    Applying KCL at $V_{x}$:
    \begin{align*}
      I_{M_1} + I_{G} - I_{M_2} &= 0 \\
      I_{M_1} + I_{G} &= I_{M_2} \tag{iii}
    \end{align*}
    ...
    \begin{align*}
      I_{M_0} &= k_{n_0}(V_{gs_0} - V_{th})^2 \cdot (1 + \lambda V_{ds_0}) \\
              ...
              &= k_{n_0}\left(V_{x} - V_{ee} - V_{th}\right)^2 \quad \text{[assuming } \lambda = 0 \text{]} \tag{iv} \\ \\
      I_{M_1} &= k_{n_1}(V_{gs_1} - V_{th})^2 \cdot (1 + \lambda V_{ds_1}) \\
              ...
              &= k_{n_1}\left(V_{x} - V_{ee} - V_{th}\right)^2 \quad \text{[assuming } \lambda = 0 \text{]} \tag{v} \\ \\
      I_{M_2} &= k_{n_2}(V_{gs_2} - V_{th})^2 \cdot (1 + \lambda V_{ds_2}) \\
              ...
              &= k_{n_2}\left(V_{y} - V_{x} - V_{th}\right)^2 \quad \text{[assuming } \lambda = 0 \text{]} \tag{vi} \\
    \end{align*}
    It can be seen that,
    \begin{align*}
  ...
  \item \textbf{Small Signal Analysis}:\\
    ...
    \begin{align*}
      ...
      V_{out}(G_4 + g_{o_2}) - V_{x}(g_{o_2} + g_{m_2}) + g_{m_2}V_{y} &= 0 \tag{vii} \\
    \end{align*}
    Applying KCL at $V_{x}$:
    \begin{align*}
      ...
      V_{x}(g_{o_2} + g_{o_1} + g_{m_1} + g_{m_2}) - V_{out}g_{o_2} - g_{m_2}V_{y} &= 0 \tag{viii}
    \end{align*}
    Applying KCL at $V_{y}$:
    \begin{align*}
      V_{y}G_3 + V_{y}g_{o_0} + g_{m_0}V_{x} &= 0 \\
      V_{y}(G_3 + g_{o_0}) + g_{m_0}V_{x} &= 0 \tag{ix}
    \end{align*}
    Adding voltage source $V_s$ at $V_y$ that supplies
    a current $I_s$ into $V_y$, we get:
    \begin{align*}
      V_{y}(G_3 + g_{o_0}) + g_{m_0}V_{x} - I_{s} &= 0 \tag{ix'}
    \end{align*}
    Fourth equation arises from Voltage source:
    \begin{align*}
      V_{y} &= V_{s} \tag{x}
    \end{align*}
    Solving equations (vii), (viii), (ix') and (x),
    \begin{align*}
      V_{out}(G_4 + g_{o_2}) - V_{x}(g_{o_2} + g_{m_2}) + g_{m_2}V_{y} &= 0 \\
      V_{out}(G_4 + g_{o_2}) - V_{x}(g_{o_2} + g_{m_2}) + g_{m_2}V_{s} &= 0 \quad \text{[from (x)]} \tag{xi} \\
      \\
      V_{x}(g_{o_2} + g_{o_1} + g_{m_1} + g_{m_2}) - V_{out}g_{o_2} - g_{m_2}V_{y} &= 0 \\
      V_{x}(g_{o_2} + g_{o_1} + g_{m_1} + g_{m_2}) - V_{out}g_{o_2} - g_{m_2}V_{s} &= 0 \quad \text{[from (x)]} \\
      V_{x}(g_{o_2} + g_{o_1} + g_{m_1} + g_{m_2}) &= V_{out}g_{o_2} + g_{m_2}V_{s} \\
      V_{x} &= \frac{V_{out}g_{o_2} + g_{m_2}V_{s}}{g_{o_2} + g_{o_1} + g_{m_1} + g_{m_2}} \tag{xii} \\
    \end{align*}
    Putting (xii) in (xi),
    \begin{align*}
      ...
      \therefore \frac{V_{out}}{V_s} = \frac{-g_{m_2}(g_{o_1} + g_{m_1})}{G_4(g_{o_2} + g_{o_1} + g_{m_1} + g_{m_2}) + g_{o_2}(g_{o_1} + g_{m_1})} \tag{xiii} \\
    \end{align*}
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{Impedance}:
    \paragraph{At Input:} Applying a test current $I_{test}$
      at $V_{y}$, from equation (ix),
      \begin{align*}
        V_{y}(G_3 + g_{o_0}) + g_{m_0}V_{x} &= I_{test} \\
      \end{align*}
      Substituting (xiii) into (xii):
      \begin{align*}
        V_{x} &= \frac{V_{out}g_{o_2} + g_{m_2}V_{y}}{g_{o_2} + g_{o_1} + g_{m_1} + g_{m_2}} \\
              ...
              &= V_{y} \cdot \frac{g_{m_2}G_4}{G_4(g_{o_2} + g_{o_1} + g_{m_1} + g_{m_2}) + g_{o_2}(g_{o_1} + g_{m_1})} \tag{xiv} \\
      \end{align*}
      Substituting (xiv) into the KCL equation at $V_y$,
      \begin{align*}
    ...
    \paragraph{At Output:} Applying a test current $I_{test}$
      at $V_{out}$, from equation (vii),
      \begin{align*}
        V_{out}(G_4 + g_{o_2}) - V_{x}(g_{o_2} + g_{m_2}) + g_{m_2}V_{y} &= I_{test} \\
        V_{out}(G_4 + g_{o_2}) - V_{x}(g_{o_2} + g_{m_2}) + g_{m_2}\left(\frac{-g_{m_0}V_{x}}{G_3 + g_{o_0}}\right) &= I_{test} \quad \text{[rearraning (ix)]} \\
        V_{out}(G_4 + g_{o_2}) - V_{x}\left((g_{o_2} + g_{m_2}) + \frac{g_{m_2}g_{m_0}}{G_3 + g_{o_0}}\right) &= I_{test} \\
        V_{out}(G_4 + g_{o_2}) - V_{x}\left(\frac{(g_{o_2} + g_{m_2})(G_3 + g_{o_0}) + g_{m_2}g_{m_0}}{G_3 + g_{o_0}}\right) &= I_{test}
      \end{align*}
      Substituting (xiii) into (xii):
      \begin{align*}
        V_{x} &= \frac{V_{out}g_{o_2} + g_{m_2}V_{y}}{g_{o_2} + g_{o_1} + g_{m_1} + g_{m_2}} \\
              ...
              &= V_{out} \left(\frac{- G_4}{(g_{o_1} + g_{m_1})}\right) \tag{xvi}
      \end{align*}
      Substituting (xvi) into the KCL equation at $V_{out}$:
      \begin{align*}
...
\paragraph{Question:}
Design a Wilson Current Mirror (Sink) such that it
mirrors a current of $I_{in} = 100\mu A$ with a current
gain of 5. Assume $V_{cc} = 5V$, $V_{ee} = 0V$,
$V_{th} = 1V$, $\lambda = 0.02 V^{-1}$,
$\frac{1}{2} \mu_n C_{ox} = 100 \mu A/V^2$ and
$L = 1 \mu m$ for all transistors. Find the
output impedance and the maximum possible load
that can be connected at the output assuming $V_{L,
max} = V_{cc}$
\paragraph{Solution:}
\begin{enumerate}
