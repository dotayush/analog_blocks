...
is constant over time}. Magnetic Charge has never been observed in nature.
Thus magnetic charge density and magnetic current density are both zero. This
means that there are no sources and sinks of magnetic flux density and thus
\( \nabla \cdot \mathbf{B(x,y,z,t)} = 0 \) for all time \(t\). This is
the origin of \textbf{Gauss's Law for Magnetism}.
...
\begin{itemize}
  ...
  \item \( \mathbf{J_e(x,y,z,t)} \): electric current density function.
    ...
    \begin{align*}
      \mathbf{J_e(x,y,z,t)} = \rho_e(x,y,z,t) \mathbf{v(x,y,z,t)}
    \end{align*}
\end{itemize}
Let's have some fun with this equation.
\begin{align*}
...
\begin{align*}
  ...
  \nabla \cdot \mathbf{D(x,y,z,t_1)} &= \nabla \cdot \mathbf{D(x,y,z,t_0)} + \rho_e(x,y,z,t_1) - \rho_e(x,y,z,t_0)
\end{align*}
If you assume that at \(t_0\), the divergence of the electric flux density
function and the charge density function are both zero, then we can simplify
the above equation to:
\begin{align*}
  \nabla \cdot \mathbf{D(x,y,z,t_1)} &= \rho_e(x,y,z,t_1)
\end{align*}
...
Faraday's and Ampere's Law (alongisde Gauss Conditions) fully define any electromagnetic system
...
\begin{enumerate}
  \item \textbf{Statics}: We assume that all functions are frozen in time. This is the
    ...
    \end{align*}
  \item \textbf{Quasi-Statics}: We assume that the flux functions [\(\mathbf{B}\) \&
  ...
  \item \textbf{Equation 6}: Taking the divergence of both sides of
    ...
    \begin{itemize}
      \item \textbf{Charge Density Is Genuinely Time-Independent}:
        ...
        \begin{align*}
          0 &= -\frac{\partial \rho_e(x,y,z,t)}{\partial t} \tag{8}
        \end{align*}
        ...
        \begin{align*}
          \rho_e(x,y,z,t) = \rho_e(x,y,z) \tag{8a}
        \end{align*}
